...
const AddNewAuthorizedUser = ({ setIsModalOpen, isModalOpen }) => {
  const [excelSheet, setExcelSheet] = useState();
  const [loading, setLoading] = useState(false);
  const { mutateAsync: UploadAuthorizesUser } = usePost();
  ...
  const handleUploadExcel = async () => {
    if (excelSheet) {
      ...
      try {
        ...
        const response = await UploadAuthorizesUser({
  ...
  return (
    <StyledModal
      title="Upload Authorized User List"
      open={isModalOpen}
      onCancel={closeModal}
      footer={null}
    >
...
export default AddNewAuthorizedUser;
